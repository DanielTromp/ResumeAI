...
This script crawls the Spinweb.nl website to find new vacancies and upload them to Airtable.
It performs the following tasks:
1. Login to Spinweb.nl
2. Get new vacancy listings
3. Compare with existing listings in Airtable
4. Extract vacancy details
5. Upload new vacancies to Airtable
...
def check_environment_variables():
    ...
    required_vars = {
        ...
        'AIRTABLE_TABLE1': AIRTABLE_TABLE1,
...
async def main():
    ...
    airtable.cleanup_closed_listings()
    ...
    if result.success:
        ...
        vacancy_links = set(f"https://{URL1_PROVIDER_NAME}" + link for link in re.findall(r'/aanvraag/\d+', result.html))

        existing_aanvragen_urls = airtable.get_existing_listings()
        lowest_url = airtable.get_lowest_listing_url()

        # Filter new listings based on existing listings and lowest URL
        ...
        for listing_url in new_listings.copy():
            ...
            if result.success:
                ...
                # Airtable addition with error handling in the module
